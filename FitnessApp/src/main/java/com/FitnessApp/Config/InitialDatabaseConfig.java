...
public class InitialDatabaseConfig {
	...
	public InitialDatabaseConfig(ExerciseRepository exerciseRepository, BodyPartRepository bodyPartRepository,
			EquipmentRepository equipmentRepository, TargetRepository targetRepository, ObjectMapper objectMapper) {
	...
	<T> List<T> initializeListData(Class<T> type, String path) {
		try {
			...
			objectMapper.readTree(inputStream).forEach(jsonNode -> item.getAndUpdate(listItem -> {
